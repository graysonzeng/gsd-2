...
Prompt: "Review milestone {{milestoneId}} requirements coverage. Working directory: {{workingDirectory}}. Read `{{roadmapPath}}`, `.gsd/milestones/{{milestoneId}}/{{milestoneId}}-CONTEXT.md`, and `.gsd/REQUIREMENTS.md` when present (or the equivalent requirements source in the inlined context). For each requirement, check the slice SUMMARY files under `.gsd/milestones/{{milestoneId}}/slices/` to determine if it is: COVERED (clearly demonstrated), PARTIAL (mentioned but not fully demonstrated), or MISSING (no evidence). Output a markdown table with columns: Requirement | Status | Evidence. End with a one-line verdict: PASS if all covered, NEEDS-ATTENTION if partials exist, FAIL if any missing."
