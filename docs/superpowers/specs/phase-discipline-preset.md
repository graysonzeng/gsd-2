# Auto-Mode Phase Discipline Preset (v7.2 — design draft with required kernel deltas)

> **File note** — this file keeps its git history from `composed-lite-harness-brainstorm.md` (renamed 2026-04-23 after v5→v6 pivot). v7 added a B-min 8-step profile skeleton; v7.1 applied a receiving-code-review pass on the same day that identified 8 factual and structural issues (see §16 for full list). v7.2 is a **doc-sync patch** that records the now-implemented v1.1 Admission shape on `feat/phase-discipline-preset-v1` without changing the broader PR-3a/PR-3b `main`-landing analysis. **The document remains a design draft — not accepted for implementation on `main` — because landing still depends on the contract-sync residue described below.** The v1–v5 history is compressed to §16's changelog with one-line entries. Two orthogonal specs were split out in the v6 rewrite:
>
...
|---|---|---|---|---|---|---|
| P0 | Admission | `discuss-milestone` | soft | — | — | v1.1: `phase-discipline-admission` post-unit hook (prompt-only checklist + retry artifacts) |
...
6. **Internal `builtin` markers are reserved for code-backed hooks only.** Prompt-only preset hooks do not carry a `builtin` marker, even though they are preset-owned. This avoids implying a runtime short-circuit path where none exists and keeps same-name user shadow hooks from being misread as built-ins.
...
### 3.3 The `"phase-discipline-8step"` preset — 3 hooks in v1 base (authoritative list)
...
**v1.1 follow-up note (implemented on `feat/phase-discipline-preset-v1`, not a change to the v1 base list above).** The admission capability is intentionally prompt-only: `phase-discipline-admission` fires after `discuss-milestone`, writes `.gsd/milestones/{milestoneId}/ADMISSION-CHECKLIST.md`, and writes `.gsd/milestones/{milestoneId}/ADMISSION-RETRY.md` if and only if the checklist ends in `Admission Decision: needs-rework`. The hook uses `max_cycles=2` and does **not** carry a `builtin` marker.
...
| **v1.1** | Admission-checklist hook on `discuss-milestone` | P0 | New **prompt-only** post-unit hook `phase-discipline-admission` with 6–8 checklist items; writes `.gsd/milestones/{mid}/ADMISSION-CHECKLIST.md`, writes `.gsd/milestones/{mid}/ADMISSION-RETRY.md` iff `Admission Decision: needs-rework`, and uses `max_cycles=2` | composed-lite `phases/p0-admission.ts` |
...
**v1.1 retry semantics note (v7.2 sync).** Because admission is prompt-driven rather than runtime-owned, `retry_on` alone is insufficient unless the stale primary artifact is cleared between cycles. The implemented contract on `feat/phase-discipline-preset-v1` is: when a prompt-driven hook requests retry and `currentCycle < maxCycles`, `rule-registry.ts` deletes the hook's main `artifact` before setting `retryPending`, so the next `checkPostUnitHooks()` call can truly re-dispatch cycle 2 instead of being skipped by idempotency.
